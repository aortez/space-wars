...
use action kind 4, version 1. Observation version 5 includes selected tool and
...
and motion. Same-build state clones retain
...
gravity, and steps Rapier once. Rendering and ray queries consume the post-step
state. The initial terrain-only query world is primed before the first frame.
Future impact handlers can enqueue field-local edits for the following tick.
...
on contact. Structural strength, impact damage, variable gravity mass, hauling,
