...
Ordinary Spacewars exposes `human`, `legacy bot`, `planner bot` and the experimental `destination bot v12` for each
...
| Destination bot v12 | `material_mission_v12` | v10 local tasks plus guarded [capture-destination selection](capture-destination-planner.md) |
