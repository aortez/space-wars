...
- M22a: ✅ Add Rapier 2D 0.34 directly with serialization and enhanced-determinism enabled, without parallel/SIMD features. Keep it isolated behind `engine-rapier` so unrelated headless scenarios such as Pizza do not depend on it. The resolved Rapier dependency graph has an effective Rust 1.89 MSRV; the Pi/Yocto toolchain remains pinned to Rust 1.94.
- M22b: ✅ Add a narrow typed adapter that owns raw Rapier worlds, sets, handles, body/collider identity, kinematic planets, radial acceleration-to-force conversion, wheel motors, suspension, contacts, articulated rover removal, and authoritative-state snapshot round trips. Rapier remains the only pose/velocity integrator for registered rover bodies; scratch-only pipeline state is reconstructed after loading.
...
- M22f: Record Pi/desktop performance and then make the explicit go/no-go decision before Spacewars integration.
- Acceptance for the feasibility gate: the rover settles without instability, drives around curved terrain under wheel torque, visibly compresses/rebounds, remains deterministic for identical inputs on the reference platform, and does not alter existing Spacewars or Pizza physics.
