...
fn assert_raster_pair_visible(path: &Path) {
        ...
        .enumerate()
    {
        ...
        if hud_regions::minimap(x, y, frame.width as usize, frame.height as usize, player)
            && pixel[0] < 100
            && pixel[1] > 130
            && pixel[2] > 100
        {
...
fn assert_raster_surface_visible(path: &Path, has_outpost: bool) {
    ...
    let mut vitals_pixels = 0;
    let mut health_pixels = 0;
    let mut minimap_planet_pixels = 0;
    let mut outpost_pixels = 0;
    let mut prompt_pixels = 0;
    for (index, pixel) in buffer[..frame.buffer_size()]
        .chunks_exact(channels)
        .enumerate()
    {
        ...
        if hud_regions::vitals(x, y, width, height, 0) {
        ...
        // Landing/transfer instructions now use a top-center contextual prompt,
        // not the old permanent amber capture row below the world.
        if hud_regions::prompt(x, y, width, height)
            && ((pixel[0] < 150 && pixel[1] > 175 && pixel[2] > 140)
                || (pixel[0] > 200 && pixel[1] > 160 && pixel[1] < 230 && pixel[2] < 100))
        {
            prompt_pixels += 1;
        }
        // P1's overview is bottom-left, separated from the vitals. Its planet
        // stays cyan even though the main scene uses a dark-blue surface.
        if hud_regions::minimap(x, y, width, height, 0)
            && pixel[0] < 100
            && pixel[1] > 140
            && pixel[2] > 110
        {
    ...
    assert!(
        prompt_pixels > 40,
        "missing top-center landing prompt: {prompt_pixels}"
    );
    assert!(
    ...
    assert!(
        vitals_pixels > 40 && health_pixels > 200,
        "missing bottom vitals: {vitals_pixels} label pixels, {health_pixels} health pixels"
    );
}
