...
#[test]
fn get_up_query_deferral_preserves_edges_and_release_cancels_pending_request() {
    for release in [false, true] {
        ...
        spaceling.apply_control_with_queries(&mut world, held, GRAVITY, DT, false);
        assert_eq!(spaceling.snapshot(&world).unwrap().get_up_attempts, 0);
        assert!(spaceling.jump_was_held);
        assert!(spaceling.pending_get_up);
        spaceling.apply_control_with_queries(
            &mut world,
            if release {
        ...
        assert_eq!(
            spaceling.snapshot(&world).unwrap().get_up_attempts,
            u64::from(!release)
        );
        assert!(!spaceling.pending_get_up);
        if !release {
            spaceling.apply_control_with_queries(&mut world, held, GRAVITY, DT, false);
            spaceling.apply_control_with_queries(&mut world, held, GRAVITY, DT, true);
