...
impl GroundSurveyJob {
    ...
    fn clear(&self, point: Vec2) -> bool {
        self.capsule.is_clear(
            &self.snapshot,
            self.world(point),
            rotation_for_direction(point.normalized().rotate_radians(self.angle)),
        )
    }
    ...
    fn reject(&mut self, id: u16, reason: GroundNodeRejection) {
        self.map.rejected.push(GroundRejectedNode { id, reason });
    ...
    fn step(&mut self) {
        ...
        match self.phase {
            ...
            Phase::NodeRay(id) => {
                ...
                let world_up = up.rotate_radians(self.angle);
                if let Some(hit) = self.ray(
                    self.position + world_up * (self.radius + 8.0),
                    -world_up,
                    self.radius + 8.0,
                ) {
                    if hit.normal.dot(world_up) < spec.min_support_alignment {
                        self.reject(id, GroundNodeRejection::SteepFloor);
                    } else {
                        self.phase = Phase::NodeCapsule(
                            GroundNode {
                                id,
                                position: (hit.point - self.position).rotate_radians(-self.angle),
                                normal: hit.normal.rotate_radians(-self.angle),
                            },
                            up,
                        );
                    }
                } else {
                    self.reject(id, GroundNodeRejection::NoRetainedFloor);
                }
            }
            Phase::NodeCapsule(node, up) => {
                if self.clear(node.position + up * standing_height()) {
                    self.map.nodes.push(node);
                    self.by_id[usize::from(node.id)] = Some(node);
                    self.phase = Phase::NodeRay(node.id + 1);
                } else {
                    self.reject(node.id, GroundNodeRejection::CapsuleObstructed);
                }
            }
            ...
            Phase::Floor(path, sample) => {
                ...
                let up = point.normalized().rotate_radians(self.angle);
                if !self
