...
impl RendererSealed for SoftwareRenderer {
    ...
    fn take_snapshot(&self) -> Result<SharedPixelBuffer<Rgba8Pixel>, PlatformError> {
        struct RestoreRotation<'a> {
            renderer: &'a SoftwareRenderer,
            rotation: RenderingRotation,
        }

        impl Drop for RestoreRotation<'_> {
            fn drop(&mut self) {
                self.renderer.set_rendering_rotation(self.rotation);
            }
        }
