...
fn position_for_rendering_rotation(
    ...
    let x = (position.x / screen_size.width).clamp(0., 1.);
    let y = (position.y / screen_size.height).clamp(0., 1.);
    let (x, y) = match rotation {
        RenderingRotation::NoRotation => (x, y),
        RenderingRotation::Rotate90 => (y, 1. - x),
        RenderingRotation::Rotate180 => (1. - x, 1. - y),
        RenderingRotation::Rotate270 => (1. - y, x),
    };

    LogicalPosition::new(x * screen_size.width, y * screen_size.height)
}
                ...
                input::Event::Pointer(pointer_event) => {
                    match pointer_event {
                        ...
                        input::event::PointerEvent::MotionAbsolute(abs_motion_event) => {
                            let mouse_pos = position_for_rendering_rotation(
                                LogicalPosition {
                                    x: abs_motion_event
                                        .absolute_x_transformed(screen_size.width as u32)
                                        as _,
                                    y: abs_motion_event
                ...
                input::Event::Touch(touch_event) => {
                    if let Some(event) = match touch_event {
                        input::event::TouchEvent::Down(touch_down_event) => {
                            self.last_touch_pos = position_for_rendering_rotation(
                                LogicalPosition::new(
...
mod tests {
    ...
    const SCREEN_SIZE: LogicalSize = LogicalSize::new(800., 480.);

    fn position(x: f32, y: f32, rotation: RenderingRotation) -> LogicalPosition {
        position_for_rendering_rotation(LogicalPosition::new(x, y), SCREEN_SIZE, rotation)
    }

    #[test]
    fn rotate_90_maps_clockwise_panel_coordinates_back_to_display_coordinates() {
        assert_eq!(
            position(800., 0., RenderingRotation::Rotate90),
            LogicalPosition::new(0., 0.)
        );
        assert_eq!(
            position(800., 480., RenderingRotation::Rotate90),
            LogicalPosition::new(800., 0.)
        );
        assert_eq!(
            position(0., 480., RenderingRotation::Rotate90),
            LogicalPosition::new(800., 480.)
        );
        assert_eq!(
            position(0., 0., RenderingRotation::Rotate90),
            LogicalPosition::new(0., 480.)
        );
    }

    #[test]
    fn absolute_position_transform_supports_every_output_rotation() {
        let point = LogicalPosition::new(200., 120.);

        assert_eq!(
            position_for_rendering_rotation(point, SCREEN_SIZE, RenderingRotation::NoRotation),
            LogicalPosition::new(200., 120.)
        );
        assert_eq!(
            position_for_rendering_rotation(point, SCREEN_SIZE, RenderingRotation::Rotate90),
            LogicalPosition::new(200., 360.)
        );
        assert_eq!(
            position_for_rendering_rotation(point, SCREEN_SIZE, RenderingRotation::Rotate180),
            LogicalPosition::new(600., 360.)
        );
        assert_eq!(
            position_for_rendering_rotation(point, SCREEN_SIZE, RenderingRotation::Rotate270),
            LogicalPosition::new(600., 120.)
        );
    }
}
